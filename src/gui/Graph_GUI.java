...
public class Graph_GUI implements ActionListener, MouseListener,Serializable
{
	graph grap;
	...
	public void paint()
	{
		if (this.grap!=null) 
		{
			...
			for(node_data node : this.grap.getV()) {
				...
				StdDraw.filledCircle(node.getLocation().x()+5, node.getLocation().y()+7, 3);
				Collection<edge_data> edd = this.grap.getE(node.getKey());
				StdDraw.setPenColor(Color.BLACK);
				StdDraw.text(node.getLocation().x()+4,node.getLocation().y()-3, ""+node.getKey());
				for (edge_data ed : edd)
				{
					...
					StdDraw.line(node.getLocation().x()+5, node.getLocation().y()+5, this.grap.getNode(ed.getDest()).getLocation().x()+5, this.grap.getNode(ed.getDest()).getLocation().y()+5);
					double x1 = node.getLocation().x()+5;
					double y1 = node.getLocation().y()+5;
					double x2 = this.grap.getNode(ed.getDest()).getLocation().x()+5;
					double y2 = this.grap.getNode(ed.getDest()).getLocation().y()+5;
					...
					StdDraw.filledCircle((xxxxx+this.grap.getNode(ed.getDest()).getLocation().x())/2, (yyyyy+this.grap.getNode(ed.getDest()).getLocation().y())/2, 3);
					StdDraw.setPenColor(Color.BLACK);

					double ans = Math.floor(ed.getWeight()*100)/100;
					StdDraw.text((int)((x1+x2)/2),(int)((y1+y2)/2),""+ans);
				}
			}
		}
	}
